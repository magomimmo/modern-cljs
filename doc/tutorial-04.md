...
`login` form and the console will still print the `Hello, World!`
string.
...
Let's now define a `validate-form` which clone the corresponding JS
`valudateForm` function.

```clj
cljs.user> (defn validate-form []
             (let [email (.getElementById js/document "email")
                   password (.getElementById js/document "password")]
               (if (and (> (count (.-value email)) 0)
                        (> (count (.-vale password)) 0))
                 true
                 (do (js/alert "please, complete the form!")
                     false))))
#'cljs.user/validate-form
```

If you now evaluate the `validate-form` symbol, you'll get JS function
generated by the CLJS compiler.

```clj
cljs.user> validate-form
#object[cljs$user$validate_form "function cljs$user$validate_form(){
var email = document.getElementById("email");
var password = document.getElementById("password");
if(((cljs.core.count.call(null,email.value) > (0))) && ((cljs.core.count.call(null,password.vale) > (0)))){
return true;
} else {
alert("please, complete the form!");
...
Now evalute the `validate-form` function. You'll get the Alert Dialog
asking you to complete the form and when you dismiss the Dialog, the
function will return the `false` value as expected.
...
Now fill the `Email Address` and the `Password` text fields and call
the `validate-form` function again.
