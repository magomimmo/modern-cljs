...
First we have to copy the static resources from the zip file you
...
```
You should end up with the following html directory layout:
...
Before going on you need to make two small changes of the
`html/login.html` file:

* deactivate the HTML5 check of the form fields by adding the
  `novalidate` attribute to the `<form>` tag;
* change the `src` attribute of the `<script>` tag from
  `"js/login.js"` to `"js/main.js"` to link the JS file which will be
...
Going back to the original JS script, the `validateForm()` function
gets `email` and `password` ids from form input and verifies that both
have a value. The `validateForm()` function returns `true` if the
