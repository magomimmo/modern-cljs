...
to the one produced by hiccups `html` macro
...
The `render-to-string` function is very useful when you want
pre-render a page using `node` on the server-side.

Considering that we still have the `domina` library available in our
project, we can simplify the above `render` call as follows:
