...
component. It follows the final solution implemented by the React
Tutorial:
...
* it calls the `onCommentSubmit` function passing to it the `author`
  and `text` trimmed values when they are not blanks and it finally
  clears the `author` and the `text` states.
...
defining its structure as simple as possible. Considering that the
code already ported to Reagent is now living in the
`modern-cljs.reagent` namespace, let's first set this namespace as the
current one in the bREPL.
...
> NOTE 4: the third `input` of the `form` is of type `button` because
