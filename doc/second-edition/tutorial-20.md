...
* first, the directory layout for the project is not so usual;
...
The `html` directory now lying in the `src/cljs` path has nothing to
...
manually. The same thing has to be done for the `:test-paths` too.
...
Here is the obtained directory layout.
...
The CLJ/CLJS programming languages already require enough efforts for
