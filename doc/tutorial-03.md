...
* `target` as the directory under which `main.js` is saved.
    ...
    > `true`. Under all other optimization settings you must specify a
    > path to where the source map will be written (extracted from
...
we had to add the `cljs-repl` task. Dont't forget to antepone the
...
From the user point of view, one the interesting aspect of `boot` is
...
`:dir` keyword argument instead of as an argument at the command-line.
...
possibile to some `boot` tasks defaults with the intent of simplifying
the structure layout of the project. That said, even if some defaults
are sane enough to be adopted for a very simple project like the one
we started from, as soons as the project get more articulated you'll
need to overwrite few of them.

Most webapps organise their HTML/CSS/JS resources to keep them
separated from eachother. Just to make an example, most of the webapps
group their JS resources under a `js` directory living in the root
directory of the html resources and link them in the corresponding
`<script>` html tag.

Let's try to adhere to this convention by first editing the
`html/index.html` file an modifying the link to the `main.js` file
...
* [`:output-to`][5]: sets the path to the JavaScript file that will be
  output
* [`:access-path`][6]: sets a path relative to the directory served by
  the webserver.
...
Without going into much details about the reason why of the way
`boot-cljs` supports the compiler options, let's be pragmatic and just
...
Then create an `cljs.edn` file in the newly created `js` directory
...
project and set the `asset-path` option to che CLJS compiler. That's it.

You can now run the CLJS compilation. To see the a very verbose output
...
[Tutorial-02][2] to verify that the our immediate feedback environment
is still working.
