...
You can safely interact we the login form which is now managed via the
`domina` lib.

Start requiring now the `modern-cljs.login` namespace and then repeat
the kind of experiments we did in [the previous tutorial][2].
...
its behaviour. Save it in `html` directory.
...
the `main.cljs.edn` file leaving in the `html/js` directory.

Open the `html/js/main.cljs.edn` file and do the addition.
...
Go back to the bREPL and require the newly create namespace to paly
...
But now there is a bad surprise attendinf for you.
...
As you see that function is still working as expected. The error is
not very informative. We have not yet introduced any debugging tools
to be used in such a case, so we try troubleshooting with what we have
in our hands.

This means that the `vaslidate-form` function is correctly called in the
...
value. `validate-form` should have been set as its value by `init`
...
What just happened has to do with the Google Closure Compiler. It gets
every CLJS file from `:source-paths` we set in the very
[first tutorial][10] and compiles all of them into the `js/main.js`
file we set in the same tutorial.
