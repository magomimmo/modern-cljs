...
namespace declaration and change the `init` function/
...
Visit the [`index.html`][12] URL. Do not fill any field (or fill just
one of them), and click the "Login" button. The application reacts by
showing you the usual `alert` window reminding you to complete the
form.  Click the `OK` button and be prepared for an unexpected result.
...
the problem of preventing the `loginForm` `action` from being fired
when the user has not filled the required fields.
...
> NOTE 5: Here, even if not needed, we wrapped the `validate-form`
> listener inside an anonymous function by passing it the event `e` to
> make clearer the mechanics of the listener. If you want, you can can
> safetly unwrap `validate-form`.
