...
* secondly, the only way to be able to define few unit tests for the
  `Enfocus` lib we should at least know its basic use cases which we
  still have to learn about.

So, here we start from where we left to improve the `Enfocus` lib even
more by:

* preparing the lib to be correctly packaged in a `jar`
* intrumenting the lib with [piggieback][5] lib;
* extending to the static resources the application of the separations
...
When in the [previous tutorial][1] we substituted the `cljx` plugin
...
`lein jar` task by first take a look at the associated help:
...
file. Aside from the `META/INF` stuff and `project.clj` file we would
like to package into the `Enfocus` jar only the source files and
resources needed by a third party developer to use the lib
itself. More precisely:

* the CLJ sources: `macros.clj` and `syntax.clj`
* the CLJS sources: `syntax.cljs`, `core.cljs`, `events.cljs` and
  `effects.cljs`.

A third party developer does not need any static resources (e.g. JS
generated files) to be able to use the `Enfocus` lib. That resources
are used for testing purpouse only.
