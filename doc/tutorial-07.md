...
because there is no more an action to be triggered.
...
listening for events during the capturing phase as well.
...
the `calc` button. The we verified via the `get-listeners` function
that there are no more listeners attached to that event.

If you click the `Calculate` button nothing happens.

Now call the `capture!` funtion to trigger the `calculate` function
during the *capture* phase of the click event. The `Calculate` button
